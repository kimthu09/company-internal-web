      ...
      <div className="py-7">
        <div className="flex flex-wrap gap-2 mx-7">
          {news.tags.map((tag) => (
            <div
              key={tag.id}
              className="rounded-full flex px-3 py-1 h-fit outline-none text-primary bg-primary/10 items-center gap-1"
            >
              {tag.name}
            </div>
          ))}
        </div>
        <h1 className="text-xl md:text-3xl font-bold text-black mt-4 mx-7">
          {news.title}
        </h1>
        <p className="mt-2 text-gray-text mt-4 mb-4 mx-7">
          Cập nhật lần cuối: {dateTimeStringFormat(news.updatedAt)}
        </p>
        <EditorBlock
          className="w-full"
          data={news.content}
          onChange={(content: OutputData) => {}}
        ...
        <div className="bg-primary/10 rounded-xl p-4 flex flex-row gap-4 mx-7 -mt-32">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="flex flex-row gap-1 text-gray-text">
              <span>
                <MailIcon />
              </span>{" "}
              <a
                href={`mailto:${news.createdBy.email}`}
